"""Backend Engineer walking skeleton (backend-engineer spec §16 Slice 1) — one keyed LLM beat.

The smallest end-to-end proof that the ``backend_engineer`` employee exists and works: seed a tiny
Python service, hire a Backend Engineer, assign a ticket, and tick the kernel once. A real model
probes the stack, implements the function + a test, runs the tests to green, the reviewed-build DoD
passes, and a ``pr`` artifact lands (the same ``pr`` lander the Engineer uses — outcome_kind matches).
...
def main() -> int:
    ...
    try:
        ...
        _log("=" * 72)
        _log("1. EMPLOYEE — materialized as backend_engineer (spec §16 Slice 1)")
