...
data that vanishes was a mock. The DoD gates on a green unit bundle, a passing api_verdict.json whose
checks include the persistence proof, AND an independently cleared code review (review_verdict.json) —
so a run can't land with the code_reviewer's findings still unresolved.
...
def main() -> int:
    ...
    try:
        ...
        # include the durability proof AND an independently cleared code review. A `persist`-named
        # check means the api_verifier actually wrote, restarted, and re-read — a mock in-memory store
        # cannot survive that. The review_verdict gate closes a real gap this smoke test used to miss:
        # a run could land with the code_reviewer's HIGH-severity findings still unresolved, because
        # nothing checked review_verdict.json before this.
        ledger.dod.create(
            "t1",
            Verifier.command(
                ...
                "test -f review_verdict.json && "
                "grep -q '\"cleared\": *true' review_verdict.json",
                artifact_class="pr",
            ),
        )
        _log(
            "\n2. TASK assigned (DoD: green bundle, a passing api_verdict.json with a persistence "
            "check, AND a cleared code review)\n   t1: " + _TASK
        )
