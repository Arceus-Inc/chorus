...
from chorus.ledger import Run, SqliteLedger, Task, TaskStatus
...
def _subtree_cost(ledger: SqliteLedger, goal_id: str) -> int:
...
def main() -> int:
    ...
    company = "toolbox"
    ledger = SqliteLedger.open(str(run_root / "ledger.db"))

    waves: list[dict[str, object]] = []
    try:
        for emp, role in [("moe", "manager"), ("ada", "engineer"), ("bob", "engineer")]:
            LedgerWorkforce(ledger.employees).hire(name=emp, role=role, reports_to="moe" if role == "engineer" else None)
        runner = build_beat_service(
            ledger, api_key=api_key, base_url=base_url, deployment=deployment, company_id=company,
            pricing=default_pricing_from_env(), seed=seed, work_root=work_root, max_concurrent_runs=2,
        )
        ...
        for n, goal in enumerate(_GOALS, 1):
            ...
            for _ in range(_MAX_TICKS_PER_GOAL):
                ...
                _log(f"   tick: goal={ledger.tasks.get(goal_id).status.value}  "  # type: ignore[union-attr]
            ...
            merged = sum(
                1 for c in children for a in arts[c.id]
                if a.resource_ref is not None and a.resource_ref.get("merged") is True
            )
            waves.append({
                "n": n, "goal": goal, "goal_id": goal_id,
            ...
            w = waves[-1]
            _log(f"   ⇒ goal={w['status']} children_done={w['children_done']}/{len(children)} "
                 f"merged={merged} main={w['main_check']} {w['elapsed']}s {w['cost']}c")

        report = _probe_and_report(ledger, run_root=run_root, company_root=company_root, repo=repo, waves=waves)
        _log(f"\nreport: {report}")
        return 0
    finally:
        ledger.close()
...
def _probe_and_report(
    ledger: SqliteLedger, *, run_root: Path, company_root: Path, repo: Path, waves: list[dict[str, object]]
) -> Path:
    ...
    # — ledger tree —
    goals = [g for g in (ledger.tasks.get(f"goal{i+1}") for i in range(len(waves))) if g is not None]
    ...
    # — company repo —
    merge_commits = len([ln for ln in _git(repo, "log", "--oneline", "--merges").splitlines() if ln])
    main_files = [ln for ln in _git(repo, "ls-files").splitlines() if ln]

    data = {
        "waves": waves, "goals": len(goals), "done_goals": done_goals,
        "children": len(all_children), "done_children": done_children, "merged_total": merged_total,
        "main_passes": main_passes, "total_cost": total_cost, "total_wall": round(total_wall, 1),
        "kinds": dict(kinds), "tools": dict(tools), "memory_tools": memory_tools,
        "mem_files": total_files, "mem_records": len(records), "mem_by_emp": dict(by_emp),
        "mem_by_scope": dict(by_scope), "mem_named_by_run": named_by_run,
        "mem_has_provenance": has_provenance, "mem_read_back": len(read_back),
        "merge_commits": merge_commits, "main_files": main_files, "records": records,
    }
    (run_root / "results.json").write_text(json.dumps(data, indent=2, default=str), encoding="utf-8")
...
def _render(d: dict[str, object]) -> str:
    ...
    mem_ok = (d["mem_read_back"] == d["mem_files"]) and (d["mem_has_provenance"] == d["mem_records"])
    ...
    child_rows = "".join(
        ...
        for w in waves for (cid, asg, st) in w["children"]  # type: ignore[union-attr]
    )
    mem_rows = "".join(
    ...
    tool_rows = "".join(f"<tr><td><code>{e(t)}</code></td><td>{c}</td></tr>"
                        for t, c in sorted(d["tools"].items(), key=lambda kv: -kv[1]))  # type: ignore[union-attr]
    kind_rows = "".join(f"<tr><td><code>{e(k)}</code></td><td>{c}</td></tr>"
                        for k, c in sorted(d["kinds"].items(), key=lambda kv: -kv[1]))  # type: ignore[union-attr]

    def chk(label: str, ok: bool, detail: str) -> str:
        return (f"<tr><td>{e(label)}</td><td class={'ok' if ok else 'bad'}>{'PASS' if ok else 'FAIL'}"
                f"</td><td>{e(detail)}</td></tr>")

    mem_checks = "".join([
        chk("one append-only record per beat", d["mem_records"] > 0, f"{d['mem_records']} records written"),
        chk("every record named by its run id", d["mem_named_by_run"] == d["mem_records"],
            f"{d['mem_named_by_run']}/{d['mem_records']} filename == run_id"),
        chk("provenance task_id + employee_id (all)", d["mem_has_provenance"] == d["mem_records"],
            f"{d['mem_has_provenance']}/{d['mem_records']} carry both"),
        chk("per-role scope (manager=team, engineer=project)", "team" in d["mem_by_scope"] and "project" in d["mem_by_scope"],  # type: ignore[operator]
            f"scopes: {d['mem_by_scope']}"),
        chk("memory written for manager AND engineers", len(d["mem_by_emp"]) >= 2, f"by employee: {d['mem_by_emp']}"),  # type: ignore[arg-type]
        chk("dream's own scanner reads back every file", d["mem_read_back"] == d["mem_files"],
    ...
    pills = "".join(f"<span class=pill>{e(p)}</span>" for p in [
