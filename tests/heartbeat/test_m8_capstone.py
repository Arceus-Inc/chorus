...
def _seed_company(ledger: SqliteLedger) -> None:
    ...
    ledger.goals.create(Goal(id="goal-capstone", title="Ship the capstone"))
    ...
    assign_task(ledger, "root", "root-lead")
...
async def test_specialist_nested_delegation_capstone_folds_verified_root_exactly_once(
    ...
    for _ in range(24):
        ...
        if ledger.tasks.get("root").status is TaskStatus.DONE:  # type: ignore[union-attr]
            break
