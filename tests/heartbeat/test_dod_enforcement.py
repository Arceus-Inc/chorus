...
def _seed(ledger: Ledger) -> Employee:
    employee = ledger.employees.create(
        Employee(id=uid("e1"), name=uid("e1"), role="backend_engineer")
    )
    ledger.tasks.submit(
...
def _scheduler(
    ledger: Ledger, beat: _RecordingBeat, employee: Employee, *, max_repair_attempts: int = 2
) -> Scheduler:
    return Scheduler(
        ledger=ledger,
...
async def _tick(ledger: Ledger, beat: _RecordingBeat, employee: Employee) -> None:
    sched = _scheduler(ledger, beat, employee)
    await sched.tick(_NOW)
    await sched.drain()


async def test_run_beat_passes_the_command_dod_as_verification(ledger: Ledger) -> None:
    employee = _seed(ledger)
    ledger.dod.create(uid("t1"), Verifier.command("pytest -q && ruff check ."))
    beat = _RecordingBeat()
...
async def test_run_beat_with_no_dod_passes_no_verification(ledger: Ledger) -> None:
...
def _scheduler_with_roles(ledger: Ledger, beat: _RecordingBeat, employee: Employee) -> Scheduler:
    from chorus.roles import RoleRegistry, default_roles
...
async def test_intake_applies_the_assignee_role_dod_when_none_set(ledger: Ledger) -> None:
...
async def test_intake_does_not_override_an_explicit_dod(ledger: Ledger) -> None:
    employee = _seed(ledger)
    ledger.dod.create(uid("t1"), Verifier.command("custom-check"))  # a human set the DoD already
    beat = _RecordingBeat()
    sched = _scheduler_with_roles(ledger, beat, employee)
...
async def test_run_beat_with_a_human_approval_dod_passes_no_verification(
    ...
    employee = _seed(ledger)
    ledger.dod.create(uid("t1"), Verifier.human_approval())  # not an objective check
    beat = _RecordingBeat()
...
async def test_pending_gate_wins_over_the_dod_so_the_task_stays_blocked(
    ...
    employee = _seed(ledger)
    ledger.dod.create(uid("t1"), Verifier.command("true"))  # a Command DoD that would pass
...
async def test_command_dod_pass_marks_done_without_an_approval(ledger: Ledger) -> None:
    employee = _seed(ledger)
...
def _recovery_wakes(ledger: Ledger) -> list[Wake]:
    return [w for w in ledger.wakes.queued() if w.reason is WakeReason.RECOVERY]


async def test_command_dod_failure_rewakes_for_self_repair(ledger: Ledger) -> None:
    employee = _seed(ledger)
    ledger.dod.create(uid("t1"), Verifier.command("pytest -q"))
    ...
    assert ledger.tasks.get(uid("t1")).status is TaskStatus.TODO  # type: ignore[union-attr]
    assert [w.payload.get("task_id") for w in _recovery_wakes(ledger)] == [uid("t1")]
    assert ledger.recovery_actions.active_for_source(uid("t1")) is None  # not escalated yet
...
async def test_command_dod_failure_escalates_when_repair_budget_is_spent(
    ...
    await sched.drain()
    assert (
        ledger.recovery_actions.active_for_source(uid("t1")) is not None
    )  # rung 3: recovery opened
...
async def test_failure_without_a_command_dod_just_blocks(ledger: Ledger) -> None:
...
async def test_non_command_dod_needs_changes_rewakes_to_continue(ledger: Ledger) -> None:
    ...
    employee = _seed(ledger)
    ledger.dod.create(uid("t1"), Verifier.agent_review(reviewer_role="reviewer", rubric="correct"))

    await _tick(ledger, _RecordingBeat(passed=False), employee)  # needs-changes, not done yet

    assert ledger.tasks.get(uid("t1")).status is TaskStatus.TODO  # type: ignore[union-attr]
    assert [w.payload.get("task_id") for w in _recovery_wakes(ledger)] == [
        uid("t1")
    ]  # re-dispatched
    assert ledger.recovery_actions.active_for_source(uid("t1")) is None  # not escalated yet


async def test_non_command_dod_escalates_when_repair_budget_is_spent(ledger: Ledger) -> None:
    employee = _seed(ledger)
    ledger.dod.create(uid("t1"), Verifier.agent_review(reviewer_role="reviewer", rubric="correct"))
    beat = _RecordingBeat(passed=False)
    sched = _scheduler(ledger, beat, employee, max_repair_attempts=1)
    ...
    await sched.drain()
    assert ledger.recovery_actions.active_for_source(uid("t1")) is not None
    assert ledger.tasks.get(uid("t1")).status is TaskStatus.BLOCKED  # type: ignore[union-attr]
