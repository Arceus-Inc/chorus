...
def test_decompose_creates_a_child(session: CliSession, ledger: SqliteLedger) -> None:
    ledger.tasks.submit(Task(id="src", intent="big"))  # depth 0
    _, out = _run("decompose src split off a piece", session)
    assert "decomposed src ->" in out


def test_decompose_at_cap_is_refused_and_blocks_the_task(
    session: CliSession, ledger: SqliteLedger
) -> None:
    ledger.tasks.submit(Task(id="src", intent="big", request_depth=5))  # at the default cap
    _, out = _run("decompose src one hop too far", session)
