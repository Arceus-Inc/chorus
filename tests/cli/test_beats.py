...
def _seed_assigned_wake(ledger: SqliteLedger, *, task_id: str, employee_id: str) -> None:
...
def test_run_tick_dispatches_a_beat_and_lands_it(ledger: SqliteLedger) -> None:
    _seed_assigned_wake(ledger, task_id="t1", employee_id="alice")
    beat = _FakeBeat(passed=True)

    report = _runner(ledger, beat).run_tick()

    assert beat.calls == ["t1"]  # the beat actually ran
    assert report.beats_started == 1
    assert ledger.tasks.get("t1").status is TaskStatus.DONE  # passed → done
    runs = ledger.runs.for_task("t1")
    assert len(runs) == 1 and runs[0].status.value == "succeeded"


def test_run_tick_blocks_the_task_on_a_failed_beat(ledger: SqliteLedger) -> None:
    _seed_assigned_wake(ledger, task_id="t1", employee_id="alice")
    beat = _FakeBeat(passed=False)

    _runner(ledger, beat).run_tick()

    assert ledger.tasks.get("t1").status is TaskStatus.BLOCKED  # failed → blocked


def test_run_tick_on_an_empty_ledger_reports_nothing(ledger: SqliteLedger) -> None:
    report = _runner(ledger, _FakeBeat()).run_tick()
    assert report.beats_started == 0 and report.wakes_dispatched == 0


def test_model_is_exposed(ledger: SqliteLedger) -> None:
