...
def test_registry_rejects_a_duplicate_handler(ledger: Ledger) -> None:
    with pytest.raises(ValueError, match="duplicate"):
        GovernanceRegistry.from_actions([TaskGateAction(ledger), TaskGateAction(ledger)])


def test_unregistered_action_is_a_governance_error(ledger: Ledger) -> None:
...
def test_request_revision_sends_a_task_gate_back_to_todo_and_wakes_assignee(
    ...
    ledger.tasks.submit(
        Task(
            id=uid("t1"), intent="ship", status=TaskStatus.IN_PROGRESS, assignee_employee_id="alice"
        )
    )
