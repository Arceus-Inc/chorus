...
def test_normalized_scope_is_sorted_regardless_of_input_order() -> None:
    result = validate_file_scope(
        parent_files_to_touch=("src/z.py", "src/a.py"),
        current_blockers=(),
        proposed_blockers=(
            ProposedBlockerScope(task_id="child", files_to_touch=("src/z.py", "src/a.py")),
        ),
        require_current_scope=False,
        require_proposed_scope=False,
    )
    ...
    assert result.valid
    assert result.parent_files_to_touch == ("src/a.py", "src/z.py")
    assert result.proposed_blockers[0].files_to_touch == ("src/a.py", "src/z.py")
