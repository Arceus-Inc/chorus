...
def _reviewed_task(ledger: SqliteLedger, *, author: str = "ada") -> CapabilityService:
    ...
    ledger.dod.create("spec", Verifier.agent_review(rubric="is it complete?", artifact_class="spec"))
    ledger.runs.create(Run(id=_REVIEW_RUN, employee_id="rev", task_id="spec", status=RunStatus.RUNNING))
...
def test_approve_records_a_passed_verdict(ledger: SqliteLedger) -> None:
    ...
    audit = [a for a in ledger.activity.by_subject("task", "spec") if a.verb is ActivityVerb.REVIEW_VERDICT]
...
def test_block_records_a_failed_verdict(ledger: SqliteLedger) -> None:
    svc = _reviewed_task(ledger)
    result = svc.record_verdict(
        task_id="spec", run_id=_REVIEW_RUN, reviewer_id="rev", approve=False, feedback="missing section 3"
    )
...
def test_a_non_agent_review_dod_is_not_reviewable(ledger: SqliteLedger) -> None:
    ...
    ledger.employees.create(Employee(id="rev", name="Rob", role="reviewer"))
    ledger.tasks.submit(Task(id="code", intent="ship code", status=TaskStatus.IN_PROGRESS,
