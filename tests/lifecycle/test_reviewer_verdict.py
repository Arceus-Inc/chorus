...
_REVIEW_RUN = uid("run_reviewer_1")


def _reviewed_task(ledger: Ledger, *, author: str = "ada") -> CapabilityService:
    ...
    ledger.dod.create(
        uid("spec"), Verifier.agent_review(rubric="is it complete?", artifact_class=uid("spec"))
    )
    ledger.runs.create(
        Run(id=_REVIEW_RUN, employee_id=uid("rev"), task_id=uid("spec"), status=RunStatus.RUNNING)
    )
    return CapabilityService(ledger)


def test_approve_records_a_passed_verdict(ledger: Ledger) -> None:
    ...
    assert dod.verdict == {"approve": True, "feedback": "solid", "reviewer": uid("rev")}
    audit = [
        a
        for a in ledger.activity.by_subject("task", uid("spec"))
        if a.verb is ActivityVerb.REVIEW_VERDICT
    ]
    assert len(audit) == 1 and audit[0].actor_employee_id == uid("rev")


def test_block_records_a_failed_verdict(ledger: Ledger) -> None:
    svc = _reviewed_task(ledger)
    result = svc.record_verdict(
        task_id=uid("spec"),
        run_id=_REVIEW_RUN,
...
def test_reviewer_cannot_verify_its_own_work(ledger: Ledger) -> None:
    # the author is also (wrongly) the reviewer — self-review is refused, nothing recorded
    svc = _reviewed_task(ledger, author=uid("rev"))
    result = svc.record_verdict(
        task_id=uid("spec"),
        run_id=_REVIEW_RUN,
        reviewer_id=uid("rev"),
...
def test_verdict_records_the_discovered_verify_command(ledger: Ledger) -> None:
    ...
    ledger.runs.create(
        Run(id=_REVIEW_RUN, employee_id=uid("rev"), task_id=uid("code"), status=RunStatus.RUNNING)
    )
...
def test_a_non_agent_review_dod_is_not_reviewable(ledger: Ledger) -> None:
    ledger.employees.create(Employee(id="ada", name="Ada", role="engineer"))
    ledger.employees.create(Employee(id=uid("rev"), name="Rob", role="reviewer"))
    ledger.tasks.submit(
        Task(
    ...
    ledger.dod.create(
        uid("code"), Verifier.command("pytest -q")
    )  # a command DoD, not a judgment gate
    ledger.runs.create(
        Run(id=_REVIEW_RUN, employee_id=uid("rev"), task_id=uid("code"), status=RunStatus.RUNNING)
    )
    result = CapabilityService(ledger).record_verdict(
        task_id=uid("code"), run_id=_REVIEW_RUN, reviewer_id=uid("rev"), approve=True, feedback="x"
    )
