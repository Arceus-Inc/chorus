...
from chorus.ledger import ActivityVerb, ApprovalAction, DodStatus, Ledger, Task, TaskStatus
...
def _task_with_manager(ledger: Ledger, verifier: Verifier) -> None:
    ...
    ledger.dod.create(uid("t1"), verifier)


def test_manager_tighten_applies_immediately(ledger: Ledger) -> None:
    ...
    assert (
        ledger.dod.verifier_for_task(uid("t1")).verification_steps()[0].command
        == "pytest && ruff check"
    )  # type: ignore[union-attr]
    verbs = [a.verb for a in ledger.activity.by_subject("task", uid("t1"))]
    assert ActivityVerb.DOD_REVISED in verbs


def test_non_manager_cannot_revise(ledger: Ledger) -> None:
    _task_with_manager(ledger, Verifier.command("pytest"))
...
def test_no_change_is_rejected(ledger: Ledger) -> None:
    _task_with_manager(ledger, Verifier.command("pytest"))
...
def test_tighten_does_not_rejudge_a_recorded_verdict(ledger: Ledger) -> None:
    _task_with_manager(ledger, Verifier.command("pytest"))
    dod = ledger.dod.get_for_task(uid("t1"))
...
def test_loosen_stages_the_proposal_and_opens_a_gate(ledger: Ledger) -> None:
    ...
    steps = ledger.dod.verifier_for_task(uid("t1")).verification_steps()  # type: ignore[union-attr]
