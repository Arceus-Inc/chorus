...
:class:`TestEvidenceTool.execute` wraps them in the harness observation + recovery contract. No keys,
...
def _run(tool: TestEvidenceTool, ctx: object, payload: dict[str, object]) -> object:
    return asyncio.run(tool.execute(payload, ctx))  # type: ignore[arg-type]


class TestTestEvidenceTool:
    def test_complete_bundle_reports_success(self, tmp_path: Path) -> None:
        _complete(tmp_path)
        result = _run(TestEvidenceTool(), _ctx(tmp_path), {})
    ...
    def test_bad_input_is_rejected(self, tmp_path: Path) -> None:
        result = _run(TestEvidenceTool(), _ctx(tmp_path), {"summary_min_words": 0})
...
class TestWiring:
    def test_capability_tool_registers_test_evidence_without_a_ledger(self) -> None:
        ...
        assert "test_evidence" in _LEDGER_FREE_CAPABILITY_TOOLS
        tool = _capability_tool("test_evidence", None)
        assert isinstance(tool, TestEvidenceTool)

    def test_identity_mapped_for_the_subagent_projection(self) -> None:
        from chorus_harness._factory import dream_tool_names

        assert dream_tool_names(("test_evidence",)) == ("test_evidence",)
