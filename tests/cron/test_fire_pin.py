...
def _seed(
    ...
    *,
    ...
    return ledger.routine_triggers.create(
        RoutineTrigger(
            id=uid("t1"), routine_id=uid("r1"), cron_expression="0 * * * *", next_run_at=next_run_at
        )
    )
...
def test_fire_pins_the_head_revision_and_sources_intent_from_it() -> None:
    ...
    try:
        ...
        task_id = fire_routine(ledger, ledger.routine_triggers.get(uid("t1")), now=_NOW)
        assert task_id is not None

        (run,) = ledger.routine_runs.by_routine(uid("r1"))
        assert run.routine_revision_id == uid("rrev1")  # pinned to the live head
...
def test_an_inflight_edit_never_rejudges_a_recorded_firing() -> None:
    ...
    try:
        ...
        rev2 = revise_routine(
            ledger, routine_id=uid("r1"), revised_by=uid("e1"), intent_template="v2"
        )
        assert rev2.revision_no == 2

        # The already-recorded run + its task keep the definition they fired under.
        (run1,) = ledger.routine_runs.by_routine(uid("r1"))
        assert run1.routine_revision_id == uid("rrev1")
        assert ledger.tasks.get(first_task).intent == "v1"  # type: ignore[union-attr]

        # The next firing picks up the new head.
        second_task = fire_routine(ledger, ledger.routine_triggers.get(uid("t1")), now=_LATER)
        assert second_task is not None
        second = ledger.tasks.get(second_task)
        assert second is not None and second.intent == "v2"
        run2 = next(r for r in ledger.routine_runs.by_routine(uid("r1")) if r.id != run1.id)
