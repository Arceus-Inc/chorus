...
class TestDecisionRepo:
    def test_create_and_get_round_trip(self, ledger: SqliteLedger) -> None:
        ledger.decisions.create(_decision())
        got = ledger.decisions.get("d1")
    ...
    def test_get_missing_returns_none(self, ledger: SqliteLedger) -> None:
        assert ledger.decisions.get("nope") is None

    def test_for_task_returns_all_for_the_task(self, ledger: SqliteLedger) -> None:
        ledger.decisions.create(_decision(decision_id="d1"))
        ledger.decisions.create(_decision(decision_id="d2"))
        ledger.decisions.create(_decision(decision_id="d3", task_id="other"))
        assert {d.id for d in ledger.decisions.for_task("t1")} == {"d1", "d2"}

    def test_supersede_is_a_pointer_not_a_mutation(self, ledger: SqliteLedger) -> None:
        ledger.decisions.create(_decision(decision_id="d1"))
        ledger.decisions.create(_decision(decision_id="d2"))  # the successor
        ledger.decisions.set_superseded_by("d1", "d2")
        assert ledger.decisions.get("d1").superseded_by == "d2"
        assert ledger.decisions.get("d2").superseded_by is None  # d2 stays live


class TestClaimRepo:
    def test_create_and_batch_fetch_for_decisions(self, ledger: SqliteLedger) -> None:
        ledger.decisions.create(_decision(decision_id="d1"))
        ledger.decisions.create(_decision(decision_id="d2"))
        ...
        ledger.claims.create(
            Claim(
                id="c3", decision_id="d2", text="unrelated", source_url="https://c", confidence=0.5
            )
        )
        batched = ledger.claims.for_decisions(["d1", "d2"])
        assert {c.id for c in batched} == {"c1", "c2", "c3"}
        assert {c.id for c in ledger.claims.for_decisions(["d1"])} == {"c1", "c2"}

    def test_for_decisions_empty_input_returns_empty(self, ledger: SqliteLedger) -> None:
