...
MANAGER_BRIEF = (
    ...
    "CARRY THE FEEDBACK FORWARD. When you react to a child that `failed` or is `blocked`, open that "
    "child's `dod_verdict`, `latest_run_summary`, and `latest_run_outcome` in the packet and copy the "
    "SPECIFIC failing checks into the corrective task's `intent` as an explicit 'Fix these exact items "
    "to pass:' list — name the failing test, the missing file, or the rejected behaviour verbatim. The "
    "next attempt must inherit the concrete reason the last one failed so it can pass, not start blind "
