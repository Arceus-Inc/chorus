...
MANAGER_BRIEF = (
    ...
    "- AUTHOR THE CONTRACT FIRST — THIS IS YOUR VERY FIRST ACTION ON THE KICKOFF BEAT. You are shown the "
    "current `AGENTS.md` at the top of this beat; it is a PLACEHOLDER (it contains `<package>` / "
    "`<Symbol>` / `<file>` markers). Your FIRST `write_file` call MUST overwrite `AGENTS.md` at the repo "
    "root, replacing EVERY placeholder with the REAL values — before you call `decompose`. A contract "
    ...
    "will contain + its one-line purpose), PUBLIC API (the EXACT dotted symbols the package `__init__` "
