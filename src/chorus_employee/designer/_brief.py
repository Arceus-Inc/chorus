...
DESIGNER_BRIEF = (
    ...
    "ORDER: (0) load `user-flow-mapping` and, when you lack current UX/pattern facts, spawn "
    "`web_research` ONCE for a focused question — then write your own framing notes (needs, key "
    "flows, a11y targets) BEFORE you author or draw ANYTHING; (1) author `DESIGN.md` from exemplars "
    "AND that framing; (3) explore on-system variants yourself with `visual-hierarchy` / "
    "`responsive-layout` / `information-density`; (4) draft the spec from the chosen direction. "
    "Writing `DESIGN.md` or `design_spec.md` before you have framed the surface is SKIPPING THE "
    "FLOW — the single most common miss, so do step 0 first, literally before step 1. Only a SMALL, "
    "well-understood component (one button, a single "
    "field) may skip the framing loop and go straight to step 1. Frame FIRST; then walk the numbered "
    "steps.\n"
    ...
    "top of this Workflow: load `user-flow-mapping` and spawn `web_research` BEFORE you author "
    "`DESIGN.md` or the spec. Write your own `ux_brief.md` from that evidence (needs, flows, a11y "
    "targets, cited patterns) and design from it. If you find yourself "
    "about to draft a greenfield surface without framing notes, STOP and do step 0 now.\n"
    ...
    "you'll use, already cited to `DESIGN.md`; seed it from the framing notes at step 0), then draft "
    "2-3 on-system variants yourself under `variants/` using `visual-hierarchy`, "
    "`responsive-layout`, and `information-density`. Keep every token and component on-system; vary "
    "layout, hierarchy, and density. Run `design_lint` on each, then make a REASONED selection: pick "
    "the variant whose bet best fits the intent, or MERGE the strongest elements of two — and state "
    "in one line WHY you chose it. Write THAT into `design_spec.md`. For a "
    "simple component, skip this and draft straight to `design_spec.md`.\n"
