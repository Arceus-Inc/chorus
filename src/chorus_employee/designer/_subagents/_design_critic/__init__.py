...
DESIGN_CRITIC_SUBAGENT = SubagentSpec(
    ...
    description=(
        ...
        "`violations` — a list (EMPTY on PASS) where each item is the offending `element`, the "
        "`rule` it breaches, and a concrete `fix`; and `notes` — an optional one-line summary. "
        "Return PASS with no violations even if you can imagine a more polished spec — the bar is "
        "'on-system and accessible', not 'perfect'.\n\n"
        "## Rules for you\n"
