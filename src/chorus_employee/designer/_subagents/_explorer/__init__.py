...
EXPLORER_SUBAGENT = SubagentSpec(
    ...
    description=(
        ...
        "them genuinely different along a NAMED axis — vary the LAYOUT (list vs grid vs split), the "
        "HIERARCHY (what leads), the INTERACTION model (inline vs modal vs wizard), and the DENSITY. "
        "Do NOT just reword the seed; each variant must make a DISTINCT design bet.\n"
        ...
        "and `variants` — one entry per variant with its `file`, a one-line `approach`, a `rationale` "
        "(the design BET it makes, the distinguishing axis, and the tradeoff it accepts — this is what "
        "lets the designer choose with reason, not a coin flip), and `design_lint_clean` (true when "
        "its design_lint came back with no findings).\n\n"
        "## Hard rules\n"
