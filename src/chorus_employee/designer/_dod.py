...
PM, its DoD **leads with a deterministic floor** (exactly as the Marketer's does): the ``design_spec.md``
exists, is substantive, and documents the three evidence sections that make "on-system + accessible"
checkable at all — its **tokens/components**, its **states** (empty/loading/error), and its
**accessibility** notes (focus order, ARIA, contrast, touch targets). A screen that looks great but omits
its states or its a11y notes is not done.
...
# she has no run_command). The spec exists, is non-empty, is substantive (>= 150 words), AND carries the
# three evidence sections that make the design auditable: a tokens/components section, a states section
# (empty/loading/error), and an accessibility section (a11y / focus / aria / keyboard / contrast). The
# in-beat Design-Critic owns design quality; the DoD owns "a real, on-system, accessible spec landed".
_DOD_COMMAND = (
    f"test -s {DESIGN_SPEC_DOC} "
    f'&& test "$(wc -w < {DESIGN_SPEC_DOC})" -ge 150 '
    f"&& grep -qiE '^#+[[:space:]]*(tokens|components|component|design system|system)' {DESIGN_SPEC_DOC} "
    f"&& grep -qiE '^#+[[:space:]]*(states|state|empty|loading|error)' {DESIGN_SPEC_DOC} "
    f"&& grep -qiE '^#+[[:space:]]*(accessibility|a11y|focus|aria|keyboard|contrast)' {DESIGN_SPEC_DOC}"
)
