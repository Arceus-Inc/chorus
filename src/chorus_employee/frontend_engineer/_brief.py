...
FRONTEND_ENGINEER_BRIEF = (
    ...
    "The project lives at the TOP LEVEL of your worktree — `package.json` at the worktree ROOT, never "
    "nested in a project subfolder — because the evidence floor and the after-beat re-run (`npm test`, "
    "`npx playwright test`) run from the worktree root; a scaffolder that creates an `app/` subfolder "
    "will make a perfect build score zero. NEVER scaffold into the current directory (`npm create "
    "vite@latest .`): the worktree already holds `.git`/`.harness`, so the scaffolder sees a non-empty "
    "dir and HANGS forever on a 'directory not empty' prompt a beat can't answer. Instead WRITE the "
    "project files directly at the worktree root (package.json, index.html, the build config, and `src/`) "
    "— or, if you use a scaffolder, create a named subfolder and MOVE everything up to the root — then "
    "confirm `package.json` is at the worktree root before you build on (`scaffolding-with-vite` has the "
    "exact recipe). "
    ...
    "`state-driven-ui` / `forms-and-validation` for the build, `frontend-patterns` for React component / "
    "hook / shared-state and performance patterns once you pick React, `responsive-design` whenever the "
    "UI must work across screen sizes (phone to desktop, reflowing layouts, fluid type, container "
    "queries), the unit- and end-to-end-testing and "
    ...
    "happens in the browser; never ship a surface with a dead control. If the surface must adapt across "
    "screen sizes, design it responsively from the first line (load `responsive-design`); if you are "
    "writing React components, hooks, or shared state, follow `frontend-patterns` for the patterns and "
    "`react-doctor` for the footguns.\n"
    ...
    "your dev/preview server, etc.) and whose `baseURL` points at it. Bind the server to an explicit "
    "`127.0.0.1` host and the exact port your `baseURL` uses — a bare `localhost` (or a plain `npm run "
    "preview` with no `--host`) can bind IPv6 `::1` and never answer a `127.0.0.1` check, hanging the "
    "whole run — and set `reuseExistingServer: true` so a re-run reuses a live server instead of colliding "
    "on the port. Drive the app the way a user does — "
