...
FRONTEND_ENGINEER_BRIEF = (
    ...
    "5. REVIEW UNDER PRESSURE. Before you declare done, spawn the read-only `code_reviewer` via "
    "`spawn_subagent` (correctness against the intent, stack fit, accessibility, whether the tests "
    "genuinely prove anything). UI proof is yours: load `playwright-e2e-authoring` and "
    "`test-evidence-discipline`, run `evidence_scan`, and use Dream `verify` if you need an isolated "
    "re-check. The reviewer cannot edit — fix every blocker and major "
    "yourself, re-run, re-capture; a FAIL is as real as a red test.\n\n"
    # — ending discipline —
