...
FRONTEND_ENGINEER_BRIEF = (
    ...
    "Do not gold-plate; do not stop short of actually working. You carry authored craft playbooks — load "
    "the one that fits the step you're on with the `skill` tool (`spec-to-working-app` for sizing; "
    "`semantic-html-and-aria` / `keyboard-and-focus` / `color-and-contrast` for accessibility; "
    "`state-driven-ui` / `es-module-architecture` / `forms-and-validation` for the build; "
    "`unit-testing-with-node-test` / `playwright-e2e-authoring` / `web-first-assertions` for the tests; "
    "`test-evidence-discipline`, `debugging-failing-tests`, `package-and-run-hygiene` for the rest) — "
    "they carry the details this brief only summarises.\n"
