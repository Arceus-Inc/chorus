...
FRONTEND_ENGINEER_BRIEF = (
    ...
    "8. VERIFICATION IS AUTOMATIC — do NOT try to run the final gate yourself. After the beat, the system "
    "checks IN THE WORKTREE that the app entry, the unit tests, the e2e tests, and the `test_evidence/` "
    "bundle all exist and are substantive — and it RE-RUNS your suites against the shipped code. Just "
    "make each deliverable real and green up front, then STOP. Never write a `verify.sh`; never try to "
    "second-guess the gate's shell.\n\n"
