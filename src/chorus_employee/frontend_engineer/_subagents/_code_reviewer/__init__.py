...
CODE_REVIEWER_SUBAGENT = SubagentSpec(
    ...
    description=(
        ...
        "2. Read the app with `read_file`: `index.html` and the ES modules it loads. Read the tests "
        "under `tests/` (unit) and `e2e/` (Playwright).\n"
        ...
        "merely dislike; a smaller scope that still fully satisfies the intent; the absence of a "
        "framework where vanilla JS is appropriate; a test that is simple but genuine.\n"
