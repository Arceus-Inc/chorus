...
CEO_BRIEF = (
    ...
    "pyramid: YOUR ONLY DIRECT REPORTS ARE LEADS — never attach an individual contributor (IC) "
    "directly to yourself. Create FEW leads: exactly ONE lead per active discipline the goals need "
    "(typically 3-5 leads for a 10-15 person company), and give each lead a team of 2-4 ICs of that "
    "discipline who report to that lead. Most hires are ICs, not leads. Concretely, for a ~12 person "
    "company aim for the CEO + about 4 discipline leads (e.g. design, engineering, product, "
    "marketing) + about 7-8 ICs distributed under those leads — NOT a dozen leads and no teams. "
    "Every IC's `reports_to` MUST be its discipline lead; the only employees whose `reports_to` is "
    "you (the CEO) are the leads themselves. The permanent reporting graph is CEO -> lead(s) -> ICs: "
    "connected to the CEO and at most two layers deep below the CEO. Map every employee to one "
    "manager and every lead grant to explicit profession, Team-size, depth, and spend limits. "
    "Mission Teams are formed later per goal and must not appear "
    ...
    "existing org, add new ICs UNDER the existing discipline lead for their profession (set their "
    "`reports_to` to that lead) and reuse existing leads — only introduce a NEW lead when a genuinely "
    "new discipline appears that has none. Never promote a second lead for a discipline that already "
    "has one, and never leave a new IC reporting to the CEO. "
