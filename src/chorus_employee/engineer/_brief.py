...
ENGINEER_BRIEF = (
    ...
    "BUILD TO THE CONTRACT'S DATA MODEL. `AGENTS.md` (on main, in your worktree) has a `## Data model` "
    "section declaring the EXACT fields of every core type and the input shape of every public function. "
    "If your module owns one of those types or the parser (e.g. a `Judgment` dataclass, or "
    "`ingest_judgments`), implement it with the EXACT field names, constructor signature, and accepted "
    "input keys the contract declares — do not rename fields, reorder a constructor, or require a "
    "different input shape. The acceptance test was written against that declared shape; if your type "
    "disagrees, the goal's gate rejects valid inputs at construction and nothing downstream runs. When "
    "the contract's data model is ambiguous, match what `acceptance/` actually constructs. "
