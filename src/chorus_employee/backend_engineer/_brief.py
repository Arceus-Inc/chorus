...
BACKEND_ENGINEER_BRIEF = (
    ...
    "(1) TEST-FIRST. Sketch the contracts, then delegate the failing tests via "
    "`spawn_subagent(subagent_type=\"test_author\", goal=...)` — never forge `test_plan.json` "
    "yourself: the specialist writes the tests, sees them fail RED, proves it with `test_red`, "
    ...
    "(4) INDEPENDENT ADVERSARIES. For a running service, "
    "`spawn_subagent(subagent_type=\"api_verifier\", goal=...)` — it boots the service and probes "
    "it over real HTTP, writing `api_verdict.json` (`api_verdict.json` only when the deliverable "
    "is a running service or API). Then "
    "`spawn_subagent(subagent_type=\"code_reviewer\", goal=...)` to red-team the diff for what "
    "tests miss — a missing authorization check, an N+1 query, an injection — until "
    "`review_verdict.json` is `cleared: true`; it reviews, you fix. Do not write those verdict "
    "files yourself. Every Git-visible mutation after review invalidates that review: in a "
    ...
    "`test_evidence`, `spawn_subagent(subagent_type=\"code_reviewer\", …)` for a cleared "
    "`review_verdict.json`, then `secret_scan`. "
