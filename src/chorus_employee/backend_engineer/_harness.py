...
because the implement → run → test → fix loop is turn-hungry. The distinctive ``test_evidence`` tool,
the API-Verifier subagent, and the net-allowlist refinement land in later slices; the walking skeleton
declares only what a reviewed build needs.
"""
