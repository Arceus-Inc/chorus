...
validation sandwich. It checks her draft against the voice spec and returns a decisive
:class:`BrandVerdict`, grounding its verdict on the deterministic `brand_lint` scan and reasoning
past what mechanical rules can't catch. It never edits — Mira keeps ownership of revisions.

The return contract (:mod:`._schema`) is pydantic-authored and emitted to the spec's
``output_schema`` via :func:`brand_verdict_output_schema`, so dream validates the final message at
runtime.
"""
...
BRAND_CRITIC_SUBAGENT = SubagentSpec(
    ...
    description=(
        ...
        "5. Return a JSON object matching your output contract: `verdict` — either `PASS` or `FAIL`; "
        "`violations` — a list (EMPTY on PASS) where each item is the offending `sentence`, the "
        "`rule` it breaches, and a concrete `fix`; and `notes` — an optional one-line summary. "
        "Return PASS with no violations even if you can imagine stricter phrasing — the bar is "
        "'on-brand and honest', not 'perfect'.\n\n"
        "## Rules for you\n"
        ...
        "- If `brand_spec.md` is missing, return FAIL with a note in `notes` that no voice spec was found.\n"
    ...
    max_turns=6,
    # Runtime-enforced return contract: the typed BrandVerdict shape (verdict + violations).
    output_schema=brand_verdict_output_schema(),
)
