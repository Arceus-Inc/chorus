...
MARKETER_BRIEF = (
    ...
    "result as fact unless the spec gives you a substantiating number. Then you MUST call the "
    "`brand_lint` TOOL on the draft — `brand_lint(doc=\"content_draft.md\")`. It is a plain tool call, "
    "NOT a subagent: call it directly, never `spawn_subagent(name=\"brand_lint\")`. It deterministically "
    "returns prohibited phrases and unsubstantiated claims; FIX every finding, then call `brand_lint` "
    "again, and do NOT spawn the `brand_critic` until brand_lint returns zero findings. Clearing "
    "brand_lint first means the Brand-Critic confirms your draft rather than correcting it.\n"
