...
MARKETER_BRIEF = (
    ...
    "6c. EXECUTING AN APPROVED GO-LIVE. If you are woken on a task where you ALREADY staged a "
    "go-live (you called stage_go_live in an earlier beat), the human decision may be in: call "
    "`execute_go_live(content_type=\"blog\")` ONCE. It is fail-closed — if the gate is APPROVED it "
    "publishes the staged draft and reports the live URL; if the gate is still pending it refuses "
    "(stop and wait — never re-stage or publish around the gate); if denied, the reach is dead — do "
    "not retry, note the denial and finish. It is idempotent: one delivery per approval, a repeat "
    "call returns the existing delivery.\n"
