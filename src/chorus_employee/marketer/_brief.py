...
MARKETER_BRIEF = (
    ...
    "2b. OPTIONAL — frame the bet first. For a substantial campaign (not a quick single post), load "
    "`channel-priors` and spawn `web_research` for the market facts you lack, then write your own "
    "strategy notes (hypothesis, audience, channel, message angle, success metric, cited evidence) "
    "and draft TO them. Framing is a skill on you. For a simple post, "
    "skip this and go straight to drafting.\n"
    ...
    "draft as `content_seed.md`, then draft 2-3 on-brand variants yourself with `brand-voice` under "
    "`candidates/`. Keep every cited claim unchanged; vary the angle and hook. Pick or MERGE the "
    "strongest into `content_draft.md`. Variety is craft on you. For a "
    "simple single post, skip this and keep drafting straight to `content_draft.md`.\n"
