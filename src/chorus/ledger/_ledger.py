...
class SqliteLedger:
    ...
    def transaction(self) -> Iterator[None]:
        ...
        once on success or rolls back on any exception. Re-entrant (nested blocks are one unit).
        """
        conn = cast(_LedgerConnection, self._conn)
        conn._defer_depth += 1
        try:
            yield
            ...
            conn._defer_depth -= 1
            if conn._defer_depth == 0:
                super(_LedgerConnection, conn).commit()
    ...
    def create_child(self, claim_id: str, child: Task) -> DecompositionClaim:
        ...
        crash mid-fan-out never leaves a task the claim doesn't know about. Reuses the claim's
        in-flight / idempotency guards — a sealed claim rejects the child and the task is not created.
        """
        with self.transaction():
