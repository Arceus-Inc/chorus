-- Migration 0001 — M1 core schema (spec 01 Clusters A, C, D, F).
-- Plain SQL, applied in filename order by the runner (Postgres / golang-migrate style).
-- Immutable once shipped: never edit; add a new numbered .sql instead.
-- Written in the SQLite ∩ Postgres intersection (spec 12). The declarative copy of
-- each table lives in ../schema/; test_schema_parity asserts they agree.
