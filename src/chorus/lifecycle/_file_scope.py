...
def file_scope_violation_payload(violation: FileScopeViolation) -> dict[str, str]:
    """Compact envelope fields for one violation, omitting unset optional keys."""

    payload: dict[str, str] = {"code": violation.code.value}
    if violation.task_id is not None:
        payload["task_id"] = violation.task_id
    if violation.path is not None:
        payload["path"] = violation.path
    if violation.other_task_id is not None:
        payload["other_task_id"] = violation.other_task_id
    if violation.other_path is not None:
        payload["other_path"] = violation.other_path
    return payload
...
def _normalize_scope(
    ...
    violations.extend(_internal_overlap_violations(task_id, canonical))
    return canonical, tuple(violations)
