...
DELEGATION_BRIEF = (
    ...
    "complete set of independently deliverable child tasks. Cover the objective by DISCIPLINE, not by "
    "layer: when it spans disciplines, give each child to the specialist whose profession owns it "
    "(design/UX to a designer, research or data to an analyst, UI or app build to a frontend_engineer, "
    "service or API to a backend_engineer) so the whole Team carries the work rather than one member "
    "doing everything. Each child is still a BIG chunk — a whole module, surface, or workstream one "
    "specialist owns end to end WITH its own tests in a single beat; never split a "
