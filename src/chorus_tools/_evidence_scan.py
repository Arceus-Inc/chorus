...
class LogAssessment:
    ...
    def absent(cls) -> LogAssessment:
        ...
        return cls(
            present=False, looks_like_run=False, passed=None, failed=None, has_failures=False
        )
...
class EvidenceScanInput(BaseModel):
    """Typed contract for ``test_evidence`` — every field defaults to the framework-agnostic layout."""

    project_manifest: str = Field(
        default="package.json", description="the project manifest (any stack)"
    )
    ...
    summary_min_words: int = Field(
        default=150, ge=1, description="minimum substantive summary length"
    )


class EvidenceScanTool(BaseTool):
